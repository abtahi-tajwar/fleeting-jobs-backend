...
public class CompanySeeder {
    ...
    private final ClassPathResource resource =
            new ClassPathResource("seeds/companies/companies_seed.csv");
    ...
    public void seed() {
        try (
            ...
            for (CSVRecord row : csv) {
                ...
                String listingUrl = row.get("listing_url");

                ParserTemplateType template =
                        getParserTemplate(row.get("url_template_file_name"));

                // The company CSV is the source of truth for the listing URL.
                template.setListingUrl(listingUrl);

                CompanyEntity companyEntity = companyRepository
                        .findByListingUrl(listingUrl)
                        .orElseGet(CompanyEntity::new);

                companyEntity.setName(row.get("name"));
                companyEntity.setListingUrl(listingUrl);
                companyEntity.setEnabled(true);

                CompanyEntity savedCompany =
                        companyRepository.save(companyEntity);

                ParserTemplateEntity parserTemplateEntity =
                        parserTemplateRepository
                                .findByCompany(savedCompany)
                                .orElseGet(() -> {
                                    ParserTemplateEntity newEntity =
                                            new ParserTemplateEntity();
            ...
            throw new IllegalStateException(
                    "Unable to read the company seed file",
                    exception
            );
        }
    }
    ...
    private ParserTemplateType getParserTemplate(String filename) {
        ...
        ClassPathResource resource =
                new ClassPathResource(
                        "seeds/companies/templates/" + filename
                );
        ...
        try {
            ...
            throw new IllegalStateException(
                    "Something went wrong while reading parser template: "
                            + filename,
                    exception
            );
        }
